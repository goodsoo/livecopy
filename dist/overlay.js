...
function loadStore() {
    try {
        ...
        for (const c of JSON.parse(raw))
            store.set(storeKey(c.page, c.old, c.dup ?? 0), c);
    }
    catch {
        /* 손상된 저장값은 무시 */
    }
...
function loadMemos() {
    try {
        ...
        for (const m of JSON.parse(raw))
            memos.set(storeKey(m.page, m.anchorText), m);
    }
    catch {
        /* 손상된 저장값은 무시 */
    }
...
function memoAnchorEl(el) {
    let node = el.parentElement;
    let best = null;
    let depth = 0;
    while (node && depth < 7) {
        if (node === document.body || node.matches(MEMO_BOUNDARY))
            break;
        ...
        if (cardTag || (rounded && (bg || border || shadow)))
            best = node; // 바깥 카드까지 계속 갱신
...
function memoAnchorText(anchorEl) {
    ...
    if (fe)
        return fe.getAttribute("data-ce-orig") || "";
...
function restoreMemoMarkers() {
    ...
    for (const m of Array.from(memos.values())) {
        if (m.page !== location.pathname)
            continue;
        let match = null;
        document.querySelectorAll("[data-ce-orig]").forEach((el) => {
            if (!match && el.getAttribute("data-ce-orig") === m.anchorText)
                match = el;
        });
        if (!match) {
            document.querySelectorAll("h1,h2,h3,h4,h5,h6").forEach((h) => {
                if (!match && (h.textContent || "").trim() === m.anchorText)
                    match = h;
            });
        }
        if (match)
            memoAnchorEl(match).classList.add("ce-has-memo");
    }
}
    ...
    while (node) {
        ...
        while (sib) {
            if (/^H[1-6]$/.test(sib.tagName))
                return (sib.textContent || "").trim().slice(0, 60);
...
function qualifies(el) {
    ...
    if (el.id === PANEL_ID || el.closest(`#${PANEL_ID}`))
        return false;
    if (el.closest("#ce-memo-bar, #ce-memo-pop, #ce-memo-backdrop"))
        return false; // 메모 UI 제외
    if (el.closest(HEADER_SEL))
        return false; // 헤더는 이동 전용 → 편집 안 함
    if (el.closest("[data-ce-id]"))
        return false; // 이미 편집 대상인 조상 안이면 skip
    if (!isVisible(el))
        return false;
...
function tag(el) {
    ...
    const dup = Array.from(document.querySelectorAll("[data-ce-orig]")).filter((o) => o.getAttribute("data-ce-orig") === orig).length;
    ...
    if (isInteractive(el))
        el.classList.add("ce-interactive");
...
function onClickCapture(e) {
    ...
    if (target.closest(`#${PANEL_ID}`))
        return;
    if (target.closest("#ce-memo-bar, #ce-memo-pop, #ce-memo-backdrop"))
        return; // 메모 UI 통과
    if (target.closest(HEADER_SEL))
        return; // 헤더는 이동 허용
...
function scan(root = document.body) {
    if (!active)
        return; // 종료된 뒤 예약된 스캔은 무시
    ensureEditParam(); // 라우팅으로 빠진 ?edit 복구
    const els = root.querySelectorAll("*");
    els.forEach((el) => {
        if (el.hasAttribute("data-ce-id"))
            return;
        if (qualifies(el))
            tag(el);
    });
...
function untagHeader() {
    document.querySelectorAll("[data-ce-id]").forEach((el) => {
        if (!el.closest(HEADER_SEL))
            return;
...
function onInput(e) {
    const el = e.target?.closest?.("[data-ce-id]");
    if (!el)
        return;
...
function updatePanel() {
    ...
    if (memoCountEl)
        memoCountEl.textContent = String(memos.size);
    const total = store.size + memos.size;
    const dl = panel?.querySelector("[data-ce-dl]");
    if (dl)
        dl.disabled = total === 0;
    const reset = panel?.querySelector("[data-ce-reset]");
    if (reset)
...
function buildPanel() {
  ...
  `;
    ...
    panel.querySelector("[data-ce-reset]").addEventListener("click", () => {
        if (!window.confirm("저장된 모든 수정·메모를 지울까요? 되돌릴 수 없습니다."))
            return;
        store.clear();
        memos.clear();
        saveStore();
...
let memoBar = null; // [메모][되돌리기] 를 담는 클러스터
let memoBtn = null;
let undoBtn = null;
let memoPop = null;
let memoBackdrop = null;
let memoHoverEl = null; // 현재 hover 로 잡힌 앵커(카드/텍스트)
let memoTargetEl = null; // 모달이 편집 중인 앵커
let memoHideTimer;
let prevBodyOverflow = "";
// 말풍선 아이콘(lucide MessageSquare) + 되돌리기 아이콘(lucide RotateCcw)
const MEMO_ICON = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>';
const UNDO_ICON = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 7v6h6"/><path d="M21 17a9 9 0 0 0-9-9 9 9 0 0 0-6 2.3L3 13"/></svg>';
// 앵커(카드/텍스트) 범위 안의 편집 대상들
function scopeTargets(el) {
    return el.hasAttribute("data-ce-id")
        ? [el]
        : Array.from(el.querySelectorAll("[data-ce-id]"));
}
function scopeHasEdits(el) {
...
function positionMemoBar(el) {
    if (!memoBar)
        return;
    // 되돌리기 버튼은 되돌릴 수정이 있을 때만 노출
    if (undoBtn)
        undoBtn.style.display = scopeHasEdits(el) ? "inline-flex" : "none";
...
function onMemoMouseOver(e) {
    const t = e.target;
    if (!t?.closest)
        return;
    if (memoBackdrop?.style.display === "block")
        return; // 모달 열려있으면 바 안 뜸
    if (memoBar?.contains(t) || memoPop?.contains(t)) {
        window.clearTimeout(memoHideTimer);
        return;
    }
    const textEl = t.closest(".ce-editable");
    if (textEl) {
        window.clearTimeout(memoHideTimer);
...
function openMemo(anchorEl) {
    if (!memoPop || !memoBackdrop)
        return;
    memoTargetEl = anchorEl;
    const text = memoAnchorText(anchorEl);
    const existing = memos.get(storeKey(location.pathname, text));
    memoPop.querySelector("[data-ce-anchor]").textContent =
        text.slice(0, 60) + (text.length > 60 ? "…" : "");
    const ta = memoPop.querySelector("textarea");
    ta.value = existing?.note || "";
    memoPop.querySelector("[data-ce-memo-del]").style.display = existing ? "block" : "none";
...
function buildMemoUI() {
  ...
  `;
    ...
    memoPop.querySelector("[data-ce-memo-del]").addEventListener("click", () => {
        memoPop.querySelector("textarea").value = "";
        saveMemo();
    });
...
function onEscKeydown(e) {
    if (e.key === "Escape" && memoBackdrop?.style.display === "block")
        closeMemo();
}
...
export function initCopyEditor(config = {}) {
    if (document.getElementById(PANEL_ID))
        return; // 중복 방지
    if (config.headerSelector)
        HEADER_SEL = config.headerSelector;
